...
/* ── Pi5 parallel IQ generation ─────────────────────────────────────────
 * The original OMP pragma only covers the channel update loop (≤16 iters).
 * The real bottleneck is the isamp loop: 260,000 iters, fully sequential.
 *
 * Fix: split the epoch into N chunks (one per core). Each chunk gets a
 * snapshot of channel state at its start, advances independently, and
 * writes directly to its slice of iq_buff. No locks, no false sharing.
 */
#define PI5_NTHREADS_MAX 4
static int pi5_nthreads = PI5_NTHREADS_MAX;
...
static void gen_chunk(ChanSnap *snaps, int nch, short *out, int len, double delt)
{
    for (int s = 0; s < len; s++) {
        int ia = 0, qa = 0;
        for (int ci = 0; ci < nch; ci++) {
            ChanSnap *c = &snaps[ci];
            int tbl = (int)((c->carr_phase >> 16) & 0x1ff);
            int v   = c->dataBit * c->codeCA * c->gain;
            ia += v * cosTable512[tbl];
            qa += v * sinTable512[tbl];
            /* advance code phase */
            c->code_phase += c->f_code * delt;
            if (c->code_phase >= CA_SEQ_LEN) {
                c->code_phase -= CA_SEQ_LEN;
        ...
        out[s*2]   = (short)((ia + 64) >> 7);
        out[s*2+1] = (short)((qa + 64) >> 7);
    }
}
...
{
    { const char *_e = getenv("GPSSIM_NTHREADS");
      if (_e) { int _n = atoi(_e); if (_n >= 1 && _n <= PI5_NTHREADS_MAX) pi5_nthreads = _n; } }
	...
	/* Heap-allocate snapshot buffer to avoid stack overflow */
	ChanSnap (*pi5_snaps)[MAX_CHAN] = malloc(pi5_nthreads * sizeof(*pi5_snaps));
	if (!pi5_snaps) { fprintf(stderr, "ERROR: malloc snaps failed.\n"); exit(1); }
	...
	for (iumd=1; iumd<numd; iumd++)
	{
		...
		/* ── Pi5 parallel IQ generation ── */
		{
			/* Build epoch-start snapshots */
					ChanSnap (*snaps)[MAX_CHAN] = pi5_snaps;
			int nch = 0;
			for (i = 0; i < MAX_CHAN; i++) {
				if (chan[i].prn <= 0) continue;
				ChanSnap *s   = &snaps[0][nch];
				s->prn        = chan[i].prn;
				s->gain       = gain[i];
				s->carr_phasestep = chan[i].carr_phasestep;
				s->carr_phase = chan[i].carr_phase;
				s->code_phase = chan[i].code_phase;
				s->f_code     = chan[i].f_code;
				s->icode      = chan[i].icode;
				s->ibit       = chan[i].ibit;
				s->iword      = chan[i].iword;
				s->dataBit    = chan[i].dataBit;
				s->codeCA     = chan[i].codeCA;
				memcpy(s->ca,   chan[i].ca,   CA_SEQ_LEN * sizeof(int));
				memcpy(s->dwrd, chan[i].dwrd, N_DWRD * sizeof(unsigned long));
				nch++;
			}

			/* Pre-advance snapshots to each chunk's start sample */
			int chunk = iq_buff_size / pi5_nthreads;
			for (int t = 1; t < pi5_nthreads; t++) {
				memcpy(snaps[t], snaps[t-1], nch * sizeof(ChanSnap));
				for (int ci = 0; ci < nch; ci++)
					snap_advance(&snaps[t][ci], chunk, delt);
			}

			/* Parallel generation — one thread per chunk */
			#pragma omp parallel for schedule(static,1) num_threads(pi5_nthreads)
			for (int t = 0; t < pi5_nthreads; t++) {
				int off  = t * chunk;
				int len  = (t == pi5_nthreads-1) ? iq_buff_size - off : chunk;
				gen_chunk(snaps[t], nch, &iq_buff[off*2], len, delt);
			}

			/* Write final chunk's end state back to chan[]
			 * gen_chunk already advanced snaps[last] fully — just copy state out */
			{
				int ci = 0;
				for (i = 0; i < MAX_CHAN; i++) {
					if (chan[i].prn <= 0) continue;
					ChanSnap *s        = &snaps[pi5_nthreads-1][ci++];
					chan[i].carr_phase = s->carr_phase;
					chan[i].code_phase = s->code_phase;
					chan[i].icode      = s->icode;
					chan[i].ibit       = s->ibit;
					chan[i].iword      = s->iword;
					chan[i].dataBit    = s->dataBit;
					chan[i].codeCA     = s->codeCA;
				}
			}
		}
		...
		{
			int total8 = 2*iq_buff_size;
			#pragma omp parallel for schedule(static) num_threads(pi5_nthreads)
			for (isamp=0; isamp<total8; isamp++)
